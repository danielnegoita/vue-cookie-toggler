...
var iframesByCategories = {};
var iframes = {
  init: function init() {
    var _this = this;

    var iframes = Array.from(document.querySelectorAll(OPTIONS$1.querySelector));
    lodash.forEach(iframes, function (iframe) {
      // validate - check if srcSelector is defined
      if (!iframe.hasAttribute(OPTIONS$1.srcSelector)) {
        _this.addPlaceholder(iframe, OPTIONS$1.validation.missingSrc);

        return;
      } // validate - check if srcSelector values is set


      if (lodash.isEmpty(iframe.getAttribute(OPTIONS$1.srcSelector))) {
        _this.addPlaceholder(iframe, OPTIONS$1.validation.srcRequired);

        return;
      } // validate - check if categorySelector is defined


      if (!iframe.hasAttribute(OPTIONS$1.categorySelector)) {
        _this.addPlaceholder(iframe, OPTIONS$1.validation.missingCategory);

        return;
      } // validate - check if categorySelector value is set


      if (lodash.isEmpty(iframe.getAttribute(OPTIONS$1.categorySelector))) {
        _this.addPlaceholder(iframe, OPTIONS$1.validation.categoryRequired);
      ...
      _this.addPlaceholder(iframe);

      var category = iframe.getAttribute(OPTIONS$1.categorySelector);

      if (!lodash.has(iframesByCategories, category)) {
  ...
  enable: function enable() {
    ...
    var iframes = this.getByCategories(categories);

    if (lodash.isEmpty(iframes)) {
      return;
    }

    lodash.forEach(iframes, function (iframe) {
      _this2.enableIframe(iframe);
    });
  },
  disable: function disable() {
    var _this3 = this;

    var iframes = this.getAll();
    lodash.forEach(iframes, function (iframe) {
      _this3.disableIframe(iframe);
    });
  },
   ...
   * @param categories - array | string
   */
  update: function update(categories) {
    this.disable();
    this.enable(categories);
  },
  enableIframe: function enableIframe(iframe) {
    iframe.src = iframe.getAttribute(OPTIONS$1.srcSelector);
    iframe.style.display = 'block';
    this.removePlaceholder(iframe);
  },
  disableIframe: function disableIframe(iframe) {
    if ("" === iframe.src) {
      return;
    }

    iframe.setAttribute(OPTIONS$1.srcSelector, iframe.src);
    iframe.style.display = 'none';
    iframe.removeAttribute('src');
    this.addPlaceholder(iframe);
  },
  getAll: function getAll() {
    if (lodash.isEmpty(iframesByCategories)) {
      return null;
    }

    return lodash.flatMap(iframesByCategories, function (iframe) {
      return iframe;
    });
  },
  getByCategories: function getByCategories(categories) {
    var _this4 = this;

    if (lodash.isString(categories)) {
      return this.getByCategory(categories);
    }

    if (!lodash.isArray(categories)) {
      return null;
    }

    var result = [];
    lodash.forEach(categories, function (category) {
      var items = _this4.getByCategory(category);
  ...
  addPlaceholder: function addPlaceholder(iframe) {
    var innerHTML = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;

    if (!iframe.hasAttribute(OPTIONS$1.placeholder.selector) || !JSON.parse(iframe.getAttribute(OPTIONS$1.placeholder.selector))) {
      return;
    }

    var placeholderElement = document.createElement('div');
    placeholderElement.className = OPTIONS$1.placeholder.className;
    placeholderElement.setAttribute('data-id', iframe.getAttribute(OPTIONS$1.srcSelector));
    placeholderElement.innerHTML = OPTIONS$1.placeholder.defaultText;

    if (iframe.hasAttribute(OPTIONS$1.placeholder.textSelector)) {
      placeholderElement.innerHTML = iframe.getAttribute(OPTIONS$1.placeholder.textSelector);
  ...
  removePlaceholder: function removePlaceholder(iframe) {
    var src = iframe.getAttribute(OPTIONS$1.srcSelector);
    var placeholderElement = document.querySelector("div[data-id=\"".concat(src, "\"]"));

    if (lodash.isNull(placeholderElement)) {
      return;
    }
...
var OPTIONS = {
  ...
  validation: {
    // for developers
    wrongType: 'Missing / Wrong script type tag! Make sure you have type="text/plain" defined on the script tag.',
    missingCategory: 'Missing iframe <strong>data-category</strong> attribute.',
    categoryRequired: 'Attribute <strong>data-category</strong> has no value specified.'
  }
};
var scriptsByCategories = {};
var scripts = {
  init: function init() {
    var scripts = Array.from(document.querySelectorAll(OPTIONS.querySelector));
    lodash.forEach(scripts, function (script) {
      ...
      if (lodash.isEmpty(script.getAttribute(OPTIONS.categorySelector))) {
        console.error(OPTIONS.validation.categoryRequired);
        return;
      }

      var category = script.getAttribute(OPTIONS.categorySelector);

      if (!lodash.has(scriptsByCategories, category)) {
   ...
   * @param categories - array | string
   */
  enable: function enable(categories) {
    var _this = this;
    ...
    var scripts = this.getByCategories(categories);

    if (lodash.isEmpty(scripts)) {
      return;
    }

    var documentFragment = document.createDocumentFragment();
    lodash.forEach(scripts, function (script) {
      _this.addScriptToDOM(script, documentFragment);
    });
   ...
   */
  disable: function disable() {
    document.querySelectorAll("script[data-gdpr-target]").forEach(function (script) {
      script.remove();
    });
  },
   ...
   */
  getAll: function getAll() {
    if (lodash.isEmpty(scriptsByCategories)) {
      return null;
    }

    return lodash.flatMap(scriptsByCategories, function (script) {
      return script;
    });
  },
  getByCategories: function getByCategories(categories) {
    ...
    var result = [];
    lodash.forEach(categories, function (category) {
      var items = _this2.getByCategory(category);
   ...
   */
  addScriptToDOM: function addScriptToDOM(script, documentFragment) {
    // if it has 'src' them use postscribe - @see https://www.npmjs.com/package/postscribe
    if (script.hasAttribute('src')) {
      postscribe$1(script.parentNode, '<script data-gdpr-target="' + script.getAttribute(OPTIONS.categorySelector) + '" src="' + script.getAttribute("src") + '"></script>');
      return;
    ...
    lodash.forEach(script.attributes, function (attrib) {
      if (attrib.specified) {
    ...
    node.setAttribute('data-gdpr-target', script.getAttribute(OPTIONS.categorySelector));
...
var ITEM_NAME = 'vue-cookie-toggler-consent';
var gdprService = {
  hasConsent: function hasConsent() {
    ...
    var consent = localStorage.getItem(ITEM_NAME);

    if (!consent) {
      return false;
    }

    var item = JSON.parse(consent);

    if (!lodash.has(item, 'expires')) {
      this.clearConsent();
      return false;
    }
  ...
  saveConsent: function saveConsent(cookiesGroups) {
    if (!this.isStorageSupported()) {
      return;
    }
  ...
  isStorageSupported: function isStorageSupported() {
    try {
      var key = "gdpr_local_storage_test_key";
...
var __vue_render__$4 = function __vue_render__() {
  ...
  return _c('vue-cookie-toggler-modal', {
    attrs: {
      "name": "gdpr-modal",
      "maxWidth": 505,
      "adaptive": true,
    ...
    staticClass: "cookie-consent_modal",
    attrs: {
    ...
    staticClass: "cookieConsent-modal__content"
  }, [_c('div', {
    staticClass: "cookie-consent_modal__header"
  }, [_c('h4', {
    staticClass: "cookie-consent_modal__header-title",
    attrs: {
      "id": "cookieConsentModalLabel"
    }
  }, [_vm._t("header")], 2), _vm._v(" "), _c('hr')]), _vm._v(" "), _c('div', {
    staticClass: "cookieConsent-modal__body"
  }, [_vm._t("default")], 2), _vm._v(" "), _c('div', {
    staticClass: "cookie-consent_modal__footer"
  }, [_vm._t("footer")], 2)])])]);
};
...
var __vue_inject_styles__$4 = function __vue_inject_styles__(inject) {
  if (!inject) return;
  inject("data-v-51b39f16_0", {
    source: ".cookie-consent_modal{padding:30px;font-family:sans-serif;line-height:20px;color:#7a8990}.cookie-consent_modal:focus{outline:0}.cookie-consent_modal .cookie-consent_modal__header .cookie-consent_modal__header-title{margin:0 0 20px 0;font-size:16px;font-weight:700;color:#142f47}.cookie-consent_modal .cookie-consent_modal__header hr{height:1px;width:100%;border:0;background:#eee}.cookie-consent_modal .cookieConsent-modal__body{margin-top:20px}.cookie-consent_modal .cookie-consent_modal__footer{border:0;margin-top:25px;text-align:right}.cookie-consent_modal .cookie-consent_modal__button{padding:12px 24px;margin-right:10px;border-radius:5px;border:1px solid transparent;line-height:14px;font-size:14px;font-family:sans-serif;white-space:nowrap;vertical-align:middle;cursor:pointer}.cookie-consent_modal .cookie-consent_modal__button:nth-last-child(1){margin-right:0}.cookie-consent_modal .cookie-consent_modal__button:focus{outline:0}.cookie-consent_modal .cookie-consent_modal__button-default{background:0 0;border:2px solid #e1e7ec;color:#142f47}.cookie-consent_modal .cookie-consent_modal__button-default:hover{background-color:#e1e7ec}.cookie-consent_modal .cookie-consent_modal__button-primary{background-color:#288ede;color:#fff}.cookie-consent_modal .cookie-consent_modal__button-primary:hover{background-color:#1c73b7}",
    map: undefined,
    media: undefined
  });
};
...
var __vue_module_identifier__$4 = "data-v-51b39f16";
/* functional template */
...
var script$2 = {
  ...
  components: {
    Switches: __vue_component__$3 // VueCollapse
  ...
  methods: {
    toggleCookieGroupConsent: function toggleCookieGroupConsent(cookieGroup) {
...
var __vue_render__$2 = function __vue_render__() {
  ...
  return _c('div', {
    staticClass: "cookies-consent_list",
    attrs: {
  ...
  }, _vm._l(_vm.cookiesGroups, function (cookieGroup) {
    return _c('v-collapse-wrapper', {
      key: cookieGroup.name,
      on: {
        "onStatusChange": _vm.toggleActiveClass
      }
    }, [_c('div', {
      staticClass: "cookie-item"
    }, [_c('div', {
      staticClass: "cookie-item__heading",
      attrs: {
        "role": "tab",
      ...
      staticClass: "cookie-item__title"
    }, [_c('img', {
      staticClass: "cookie-item__title-img",
      attrs: {
      ...
      on: {
        "input": function input($event) {
          return _vm.toggleCookieGroupConsent(cookieGroup);
        }
      }
    })], 1), _vm._v(" "), _c('div', {
      directives: [{
      ...
      staticClass: "cookie-item__body"
    }, [_c('div', {
      staticClass: "cookie-item__description",
      domProps: {
        "innerHTML": _vm._s(cookieGroup.description)
      }
    })])])]);
  }), 1)], 1);
};
...
var __vue_inject_styles__$2 = function __vue_inject_styles__(inject) {
  if (!inject) return;
  inject("data-v-4516241c_0", {
    source: ".v-collapse-content[data-v-4516241c]{max-height:0;overflow:hidden;padding:0;transition:all 150ms ease}.v-collapse-content-end[data-v-4516241c]{max-height:500px;transition:all 150ms ease}.cookie-item__active .cookie-item__title-img[data-v-4516241c]{transform:rotate(180deg);transition:transform 150ms ease-out}.cookies-consent_list[data-v-4516241c]{margin-top:30px;margin-bottom:10px}.cookie-item[data-v-4516241c]{border-radius:3px;background-color:#f8f8f8;margin-bottom:10px}.cookie-item__heading[data-v-4516241c]{border:none;padding:0 15px;border-radius:3px;background-color:#f8f8f8;display:flex;justify-content:space-between;align-items:center}.cookie-item__title[data-v-4516241c]{color:#142f47;font-family:sans-serif;font-size:15px;font-weight:700;cursor:pointer}.cookie-item__title-img[data-v-4516241c]{margin-right:5px}.cookie-item__body[data-v-4516241c]{border:2px solid #f8f8f8!important;border-top:0!important;background-color:#fff;-webkit-border-bottom-right-radius:3px;-webkit-border-bottom-left-radius:3px;-moz-border-radius-bottomright:3px;-moz-border-radius-bottomleft:3px;border-bottom-right-radius:3px;border-bottom-left-radius:3px}.cookie-item__description[data-v-4516241c]{padding:16px;color:#7a8990;font-family:sans-serif;font-size:12px;line-height:20px}label.vue-switcher[data-v-4516241c]{margin-bottom:0!important}.cookie-item__heading[data-v-4516241c] .vue-switcher-theme--custom.vue-switcher-color--blue div{background-color:rgba(36,130,217,.38)}.cookie-item__heading[data-v-4516241c] .vue-switcher-theme--custom.vue-switcher-color--blue.vue-switcher--unchecked div{background-color:rgba(0,0,0,.38)}.cookie-item__heading[data-v-4516241c] .vue-switcher-theme--custom.vue-switcher-color--blue div:after{background-color:#2482d9;box-shadow:0 1px 1px 0 rgba(0,0,0,.14),0 2px 1px -1px rgba(0,0,0,.12),0 1px 3px 0 rgba(0,0,0,.2)}.cookie-item__heading[data-v-4516241c] .vue-switcher-theme--custom.vue-switcher-color--blue.vue-switcher--unchecked div:after{background-color:#fff;box-shadow:0 1px 1px 0 rgba(0,0,0,.14),0 2px 1px -1px rgba(0,0,0,.12),0 1px 3px 0 rgba(0,0,0,.2)}",
    map: undefined,
    media: undefined
  });
};
...
var __vue_scope_id__$2 = "data-v-4516241c";
/* module identifier */

var __vue_module_identifier__$2 = "data-v-4516241c";
/* functional template */
...
var script$1 = {
  name: 'EnableCookiesAlert',
  methods: {
    isStorageSupported: function isStorageSupported() {
      return gdprService.isStorageSupported();
    }
  }
};/* script */
...
var __vue_render__$1 = function __vue_render__() {
  ...
  return !_vm.isStorageSupported() ? _c('div', {
    staticClass: "cookie-enable__message"
  }, [_vm._ssrNode("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"16\" height=\"16\" viewBox=\"0 0 24 24\" fill=\"#142F47\" data-v-5c8186dc><path d=\"M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm-.001 5.75c.69 0 1.251.56 1.251 1.25s-.561 1.25-1.251 1.25-1.249-.56-1.249-1.25.559-1.25 1.249-1.25zm2.001 12.25h-4v-1c.484-.179 1-.201 1-.735v-4.467c0-.534-.516-.618-1-.797v-1h3v6.265c0 .535.517.558 1 .735v.999z\" data-v-5c8186dc></path></svg>\n    For a better experience enable cookies\n")]) : _vm._e();
};
...
var __vue_inject_styles__$1 = function __vue_inject_styles__(inject) {
  if (!inject) return;
  inject("data-v-5c8186dc_0", {
    source: ".cookie-enable__message[data-v-5c8186dc]{padding:15px 20px;background-color:#e5effa;border-radius:5px;color:#142f47;font-family:sans-serif;text-align:left}.cookie-enable__message svg[data-v-5c8186dc]{vertical-align:text-top}",
    map: undefined,
    media: undefined
  });
};
...
var __vue_scope_id__$1 = "data-v-5c8186dc";
/* module identifier */

var __vue_module_identifier__$1 = "data-v-5c8186dc";
/* functional template */
...
var script = {
  ...
  components: {
    GdprModal: __vue_component__$4,
    GdprCookiesListing: __vue_component__$2,
    EnableCookiesAlert: __vue_component__$1
  },
  props: {
    cookiesGroups: {
      type: Array,
      required: true,
      validator: function validator(groups) {
        return groups.every(function (group) {
          return lodash.isObject(group) && lodash.has(group, 'name') && lodash.has(group, 'title') && lodash.has(group, 'description') && lodash.has(group, 'allowed') && lodash.has(group, 'disabled');
        });
      }
    },
  ...
  methods: {
    ...
    isCurrentPageExcluded: function isCurrentPageExcluded() {
      if (lodash.isNull(this.excludePages)) {
        return false;
      }
    ...
    isPageExcluded: function isPageExcluded(page) {
      return !lodash.chain(this.excludePages).split(',').find(function (exPage) {
        return lodash.trim(exPage) === page;
    ...
    allowAll: function allowAll() {
      lodash.forEach(this.cookies, function (cookie) {
        cookie.allowed = true;
      });
      this.saveConsent();
    },
    ...
    updateCookieGroupConsent: function updateCookieGroupConsent(cookieGroup) {
      cookieGroup.allowed = !cookieGroup.allowed;
    },
    blockCookies: function blockCookies() {
      parsers.init();
      this.initListeners();
    },
    ...
    getAllowedCategories: function getAllowedCategories() {
      var allowedCategories = [];
      lodash.forEach(this.getConsent(), function (cookie) {
        if (cookie.allowed) {
    ...
    initListeners: function initListeners() {
      ...
      var elements = Array.from(document.querySelectorAll('[data-gdpr-action="showOptions"]'));
      lodash.forEach(elements, function (elem) {
        elem.addEventListener("click", function () {
          _this.showMoreInfoTab = true;
...
var __vue_render__ = function __vue_render__() {
  ...
  return _c('div', [!_vm.showMoreInfoTab ? _c('gdpr-modal', {
    attrs: {
    ...
    scopedSlots: _vm._u([{
      ...
      fn: function fn() {
        return [_c('button', {
          staticClass: "cookie-consent_modal__button cookie-consent_modal__button-default",
          attrs: {
            "type": "button"
          },
          on: {
            "click": function click($event) {
              _vm.showMoreInfoTab = true;
            }
          }
        }, [_vm._v("\n              More information\n            ")]), _vm._v(" "), _c('button', {
          staticClass: "cookie-consent_modal__button cookie-consent_modal__button-primary",
          attrs: {
            "type": "button"
          },
          on: {
      ...
      proxy: true
    }], null, false, 1185980829)
  }, [_vm._v(" "), _c('p', [_vm._v("\n          Use cookies that are essential to the operation of our websites. \n          These cookies are strictly necessary to provide you with services available on our websites and to use some of its features.\n        ")]), _vm._v(" "), _c('enable-cookies-alert')], 1) : _c('gdpr-modal', {
    attrs: {
      "toggleModal": _vm.toggleModal
    },
    scopedSlots: _vm._u([{
...
var __vue_module_identifier__ = "data-v-38646074";
/* functional template */
...
var component = /*#__PURE__*/(function () {
  ...
  installable.install = function (Vue) {
    Vue.component('VueCookieToggler', installable);
  };
