...
const OPTIONS = {
  ...
  placeholder: {
    ...
    textSelector: 'alt',
    defaultText:
      'To view this content you need to <a href="#" data-gdpr-action="showOptions">Enable Cookies</a>',
  },
  validation: {
    // for developers
...
export default {
  ...
  addPlaceholder(iframe, innerHTML = null) {
    if (
      !iframe.hasAttribute(OPTIONS.placeholder.selector) ||
    ...
    placeholderElement.setAttribute(
      'data-id',
      iframe.getAttribute(OPTIONS.srcSelector)
    );
    placeholderElement.innerHTML = OPTIONS.placeholder.defaultText;

    if (iframe.hasAttribute(OPTIONS.placeholder.textSelector)) {
      placeholderElement.innerHTML = iframe.getAttribute(
        OPTIONS.placeholder.textSelector
      );
    }
